...
import { IssueStatus, Project, ProjectMember, User } from "@db/schema";
import { createApp } from "@/app";

const app = createApp();
const TEST_EMAIL_PATTERN = "status-test-%@example.com";
...
async function cleanupTestData(): Promise<void> {
  ...
  if (userIds.length > 0) {
    ...
    if (projectIds.length > 0) {
      await db.delete(IssueStatus).where(inArray(IssueStatus.project_id, projectIds));
...
function uniqueEmail(): string {
  return `status-test-${Date.now()}-${Math.random().toString(36).slice(2)}@example.com`;
}
...
async function setupProject() {
  ...
  const projectResponse = await request(app)
    ...
    .send({ name: "Status Tracker Project" });
...
function statusesUrl(projectId: string): string {
  return `/api/projects/${projectId}/statuses`;
}

function createStatus(projectId: string, accessToken: string, body: Record<string, unknown> = {}) {
  return request(app)
    .post(statusesUrl(projectId))
    .set("Authorization", `Bearer ${accessToken}`)
    .send({ name: "In Review", ...body });
}

describe("Issue Status module", () => {
  beforeEach(cleanupTestData);
  afterAll(cleanupTestData);

  describe("POST /api/projects/:projectId/statuses", () => {
    it("creates a status for the project owner", async () => {
      const { owner, projectId } = await setupProject();

      const response = await createStatus(projectId, owner.accessToken, {
    ...
    it("assigns increasing positions when position is omitted", async () => {
      const { owner, projectId } = await setupProject();
      await createStatus(projectId, owner.accessToken, { name: "First" });

      const response = await createStatus(projectId, owner.accessToken, { name: "Second" });

      expect(response.body.data.position).toBe(2);
    });

    it("rejects a duplicate status name in the same project", async () => {
      const { owner, projectId } = await setupProject();
      await createStatus(projectId, owner.accessToken);
    ...
    it("rejects creation in a nonexistent project", async () => {
      ...
      const response = await createStatus(NIL_UUID, accessToken);
    ...
    it("rejects creation from a non-owner project member", async () => {
      ...
      const response = await createStatus(projectId, member.accessToken);
    ...
    it("rejects creation without an access token", async () => {
      ...
      const response = await request(app).post(statusesUrl(projectId)).send({ name: "No auth" });
    ...
    it("rejects an invalid project id", async () => {
      ...
      const response = await createStatus("not-a-uuid", accessToken);
    ...
    it("rejects a name that is too short", async () => {
      ...
      const response = await createStatus(projectId, owner.accessToken, { name: "ab" });
  ...
  describe("GET /api/projects/:projectId/statuses", () => {
    it("lists statuses for any project member", async () => {
      const { owner, projectId } = await setupProject();
      const member = await registerUser();
      await inviteMember(projectId, owner.accessToken, member.userId);
      await createStatus(projectId, owner.accessToken, { name: "First" });
      await createStatus(projectId, owner.accessToken, { name: "Second" });

      const response = await request(app)
        .get(statusesUrl(projectId))
  ...
  describe("GET /api/projects/:projectId/statuses/:statusId", () => {
    it("returns the status for a project member", async () => {
      const { owner, projectId } = await setupProject();
      const { body } = await createStatus(projectId, owner.accessToken);

      const response = await request(app)
        .get(`${statusesUrl(projectId)}/${body.data.id}`)
        .set("Authorization", `Bearer ${owner.accessToken}`);
    ...
    it("returns 404 for a missing status", async () => {
      ...
      const response = await request(app)
        .get(`${statusesUrl(projectId)}/${NIL_UUID}`)
        .set("Authorization", `Bearer ${owner.accessToken}`);
    ...
    it("rejects an invalid status id", async () => {
      ...
      const response = await request(app)
        .get(`${statusesUrl(projectId)}/not-a-uuid`)
        .set("Authorization", `Bearer ${owner.accessToken}`);
  ...
  describe("PATCH /api/projects/:projectId/statuses/:statusId", () => {
    it("updates a status name for the project owner", async () => {
      const { owner, projectId } = await setupProject();
      const { body } = await createStatus(projectId, owner.accessToken);

      const response = await request(app)
        .patch(`${statusesUrl(projectId)}/${body.data.id}`)
        .set("Authorization", `Bearer ${owner.accessToken}`)
        .send({ name: "Renamed status" });

      expect(response.status).toBe(200);
      expect(response.body.data.name).toBe("Renamed status");
    });
    ...
    it("rejects renaming to a name that already exists in the project", async () => {
      const { owner, projectId } = await setupProject();
      await createStatus(projectId, owner.accessToken, { name: "Taken name" });
      const { body } = await createStatus(projectId, owner.accessToken, { name: "Original name" });
  ...
  describe("PATCH /api/projects/:projectId/statuses/:statusId/archive", () => {
    it("archives a status for the project owner", async () => {
      const { owner, projectId } = await setupProject();
      const { body } = await createStatus(projectId, owner.accessToken);

      const response = await request(app)
        .patch(`${statusesUrl(projectId)}/${body.data.id}/archive`)
        .set("Authorization", `Bearer ${owner.accessToken}`);
  ...
  describe("PATCH /api/projects/:projectId/statuses/:statusId/restore", () => {
    it("restores an archived status", async () => {
      ...
      const response = await request(app)
        .patch(`${statusesUrl(projectId)}/${body.data.id}/restore`)
        .set("Authorization", `Bearer ${owner.accessToken}`);
    ...
    it("rejects restoring a status that is not archived", async () => {
      const { owner, projectId } = await setupProject();
      const { body } = await createStatus(projectId, owner.accessToken);
      ...
      expect(response.status).toBe(409);
      expect(response.body.error.code).toBe("STATUS_NOT_ARCHIVED");
    });
  });
});
